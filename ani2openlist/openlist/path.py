...
class OpenlistPath(BaseModel):
    ...
    base_path: str  # 用户基础路径（用于计算文件/目录在 Openlist 服务器上的绝对地址）
    full_path: str  # 相对用户根文件/目录路径

    id: str | None = None  # 文件/目录 ID（Openlist V3.45）
    path: str | None = None  # 相对存储器根目录的文件/目录路径（Openlist V3.45）
    name: str  # 文件/目录名称
    size: int  # 文件大小
    is_dir: bool  # 是否为目录
    modified: str  # 修改时间
