...
from ani2openlist.openlist.path import OpenlistPath
from ani2openlist.openlist.storage import OpenlistStorage


class OpenlistClient(metaclass=Multiton):
    """
    Openlist 客户端 API
    """
    ...
    def __init__(
        ...
        :param url: Openlist 服务器地址
        :param username: Openlist 用户名
        :param password: Openlist 密码
        :param token: Openlist 永久令牌
        """
    ...
    async def __request(
        ...
        **kwargs,
        ...
        :param method 请求方法
        :param url 请求 url
        :param auth header 中是否带有 openlist 认证令牌
        """
    ...
    def __get_token(self) -> str:
        ...
        if self.__token["expires"] == -1:
            ...
            if self.__token["expires"] < now_stamp:  # 令牌过期需要重新更新
                ...
                )  # 2天 - 5分钟（openlist 令牌有效期为 2 天，提前 5 分钟刷新）

            return self.__token["token"]

    def api_auth_login(self) -> str:
        """
        登录 Openlist 服务器认证账户信息
    ...
    async def async_api_fs_list(self, dir_path: str) -> list[OpenlistPath]:
        ...
        return [
            OpenlistPath(
                ...
                full_path=dir_path + "/" + openlist_path["name"],
                **openlist_path,
            )
            for openlist_path in result["data"]["content"]
        ]
    ...
    async def async_api_admin_storage_list(self) -> list[OpenlistStorage]:
        ...
        return [OpenlistStorage(**storage) for storage in result["data"]["content"]]

    async def async_api_admin_storage_create(self, storage: OpenlistStorage) -> None:
        """
        创建存储 需要管理员用户权限

        :param storage: OpenlistStorage 对象
        """
    ...
    async def get_storage_by_mount_path(
        ...
        :param kwargs: 创建存储器 OpenlistStorge 时的参数
        :return: OpenlistStorage 对象
        """

        for storage in await self.async_api_admin_storage_list():
            if storage.mount_path == mount_path:
                return storage
        logger.debug(f"在 Openlist 服务器上未找到存储器 {mount_path}")

        if create:
            kwargs["mount_path"] = mount_path
            storage = OpenlistStorage(**kwargs)
